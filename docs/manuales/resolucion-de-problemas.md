...
Guía rápida para los problemas más comunes al usar Stoka LMS. No necesitás conocimientos técnicos
para seguirla. Si después de probar lo de acá el problema sigue, contactá primero al **coordinador
...
- **"Usuario o contraseña incorrectos"**: revisá que estés escribiendo bien tu email (sin espacios) y
...
- **Olvidé mi contraseña**: hoy la plataforma no tiene un botón de "olvidé mi contraseña" que te la
  restablezca sola. Pedile al administrador de tu institución que te asigne una contraseña nueva.
- **"Este dominio no corresponde a ninguna institución registrada"**: probablemente estés entrando por
  una dirección (URL) que no es la de tu institución. Confirmá con tu coordinador cuál es la dirección
  correcta.
- La pantalla de acceso nunca aparece, o tarda muchísimo: puede ser un problema de conexión a
  internet, o que el sistema esté temporalmente fuera de servicio — probá de nuevo en unos minutos y,
  si sigue igual, avisá a tu institución.

## Entré, pero no veo ningún curso ni matrícula

- Si sos estudiante y "Mis matrículas" aparece vacío: todavía no te matricularon en ningún curso.
  Pedile al coordinador académico que revise tu matrícula.
- Si sos docente o coordinador y "Cursos" aparece vacío: puede que tu institución todavía no haya
  cargado ningún curso, o que tu cuenta no tenga el rol correcto asignado — consultá con el
...
Si creés que SÍ deberías poder hacer esa acción, pedile al administrador de tu institución que revise
qué rol tenés asignado.

## No puedo emitir un certificado

- **"Esta matrícula está en estado... no 'completed'"**: solo se pueden emitir certificados para
  matrículas marcadas como **Completado**. Marcá primero al estudiante como completado en la sección
...
- **"Esta matrícula ya tiene un certificado vigente"**: esa persona ya tiene un certificado activo para
  ese curso. Si necesitás emitir uno nuevo (por ejemplo, corregir un error), primero hay que
  **revocar** el certificado anterior.

## El código de verificación de un certificado dice "no encontrado"

- Revisá que copiaste el código completo, sin espacios extra al principio o al final.
- Si escaneaste un código QR y da error, puede que la imagen esté dañada o incompleta — pedile a la
...
- Si el certificado dice "Revocado" en vez de "no encontrado", significa que existe pero ya no es
  válido — no es un error de tu parte, contactá a la institución que lo emitió si tenés dudas.

## No puedo descargar el PDF de un certificado

El enlace de descarga es temporal (por seguridad, deja de funcionar después de un tiempo). Volvé a la
...
normal que te pida iniciar sesión de nuevo — solo repetí los pasos de
...
hiciste mal vos. Anotá (o hacé una captura de pantalla de) el mensaje completo y avisale a quien
